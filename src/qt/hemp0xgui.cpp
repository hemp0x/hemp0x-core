...
const std::string Hemp0xGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString Hemp0xGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "HEMPBTC"  , 1,          8},
    { "mBTC",   "HEMPBTC"  , 1000,       5},
    { "µBTC",   "HEMPBTC"  , 1000000,    2},
    { "Satoshi","HEMPBTC"  , 100000000,  0},
    { "USDT",   "HEMPUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(Hemp0xGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

Hemp0xGUI::Hemp0xGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void Hemp0xGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Hemp0x address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and hemp0x: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to HEMP addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Hemp0x addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Hemp0x addresses"));
    ...
    openAction->setStatusTip(tr("Open a hemp0x: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Hemp0x command-line options").arg(tr(PACKAGE_NAME)));
...
void Hemp0xGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/hemptext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/hemp0xtext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* hemp0xLabelLayout = new QVBoxLayout(toolbarWidget);
        hemp0xLabelLayout->addWidget(labelToolbar);
        hemp0xLabelLayout->addWidget(m_toolbar);
        hemp0xLabelLayout->setDirection(QBoxLayout::TopToBottom);
        hemp0xLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Hemp0x Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/Hemp0xProject/Hemp0x/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Hemp0x release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/Hemp0xProject/Hemp0x/releases";
...
void Hemp0xGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/hemptext")));
        m_toolbar->setMaximumWidth(65);
...
void Hemp0xGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool Hemp0xGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool Hemp0xGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void Hemp0xGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void Hemp0xGUI::setWalletActionsEnabled(bool enabled)
{
...
void Hemp0xGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void Hemp0xGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void Hemp0xGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void Hemp0xGUI::showDebugWindow()
{
...
void Hemp0xGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void Hemp0xGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void Hemp0xGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void Hemp0xGUI::openClicked()
{
...
void Hemp0xGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void Hemp0xGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void Hemp0xGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void Hemp0xGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void Hemp0xGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void Hemp0xGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** HEMP START */
void Hemp0xGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void Hemp0xGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void Hemp0xGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void Hemp0xGUI::gotoRestrictedAssetsPage()
{
...
void Hemp0xGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Hemp0x network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void Hemp0xGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void Hemp0xGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void Hemp0xGUI::updateHeadersSyncProgressLabel()
{
...
void Hemp0xGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void Hemp0xGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Hemp0x"); // default title
...
void Hemp0xGUI::changeEvent(QEvent *e)
{
...
void Hemp0xGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void Hemp0xGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void Hemp0xGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "HEMP")
        msg += tr("Amount: %1\n").arg(Hemp0xUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(Hemp0xUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void Hemp0xGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void Hemp0xGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool Hemp0xGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool Hemp0xGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void Hemp0xGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void Hemp0xGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void Hemp0xGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void Hemp0xGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void Hemp0xGUI::detectShutdown()
{
...
void Hemp0xGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void Hemp0xGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void Hemp0xGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(Hemp0xGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void Hemp0xGUI::subscribeToCoreSignals()
{
...
void Hemp0xGUI::unsubscribeFromCoreSignals()
{
...
void Hemp0xGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void Hemp0xGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<Hemp0xUnits::Unit> units = Hemp0xUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const Hemp0xUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(Hemp0xUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(Hemp0xUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (Hemp0xUnits::Unit u : Hemp0xUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(Hemp0xUnits::name(u)), this);
...
{
    setText(Hemp0xUnits::name(newUnits));
}
...
void Hemp0xGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void Hemp0xGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "Hemp0xGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void Hemp0xGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/Hemp0xProject/Hemp0x/releases"));
