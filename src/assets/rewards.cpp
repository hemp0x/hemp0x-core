...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for HEMP
    ...
    if (p_rewardSnapshot.strDistributionAsset != "HEMP") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is HEMP with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using HEMP differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "HEMP") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Hemp0x address (already validated during ownership snapshot creation)
