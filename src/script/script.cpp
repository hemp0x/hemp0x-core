...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** HEMP START */
    case OP_HEMP_ASSET              : return "OP_HEMP_ASSET";
    /** HEMP END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_HEMP_ASSET) { // OP_HEMP_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == HEMP_R) { // Check to see if HEMP starts at 27 ( this->size() < 105)
                if ((*this)[28] == HEMP_V)
                    if ((*this)[29] == HEMP_N)
                        index = 30;
            } else {
                if ((*this)[28] == HEMP_R) // Check to see if HEMP starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == HEMP_V)
                        if ((*this)[30] == HEMP_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == HEMP_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == HEMP_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == HEMP_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == HEMP_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_HEMP_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_HEMP_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_HEMP_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_HEMP_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_HEMP_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_HEMP_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
