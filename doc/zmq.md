...
The Hemp0x Core daemon can be configured to act as a trusted "border
router", implementing the hemp0x wire protocol and relay, making
...
The ZeroMQ feature in Hemp0x Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building hemp0xd:
...
For instance:
    ...
    $ hemp0xd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/hemp0xd.tx.raw
...
These options can also be provided in hemp0x.conf.
...
From the perspective of hemp0xd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into hemp0xd directly. Furthermore, no information is
...
using. Hemp0xd appends an up-counting sequence number to each
