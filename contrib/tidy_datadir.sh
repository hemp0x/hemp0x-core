...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Hemp0x database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Hemp0x datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Hemp0x datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Hemp0x 0.7 datadir."
    ;;
  3)
    echo "Detected Hemp0x pre-0.8 datadir."
    ;;
  4)
    echo "Detected Hemp0x 0.8 datadir."
    ;;
esac
